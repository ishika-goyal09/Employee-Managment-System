...
const PrintPayslip = () => {
    ...
    useEffect(()=>{
      ...
      //   setLoading(false)
      // },1000)
      api.get(`/payslips/${id}`).then((res)=>setPayslip(res.data)).catch
     ...
     <div className='grid grid-cols-2 gap-6 mb-8'>
      <div>
       ...
       <div>
        ...
        <p className='font-semibold text-slate-900'>{payslip.employee?.position}
        </p>
      </div>

       <div>
        <p className='text-xs text-slate-400 uppercase tracking-wider mb-1 '>Email</p>
        <p className='font-semibold text-slate-900'>{payslip.employee?.email}
        </p>
      </div>
      ...
      <table className='w-full text-sm'>
        ...
        <tbody>
          ...
          <tr className='border-t-2 border-slate-200 bg-slate-50'>
            ...
            <td className='text-right py-4 px-4 text-slate-900 text-lg'>-${payslip.netSalary?.toLocaleString()}
            </td>
          </tr>
        </tbody>
      </table>
